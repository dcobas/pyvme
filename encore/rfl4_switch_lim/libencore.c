...
encore_handle encore_open(char *devname, int lun)
{
	...
	snprintf(tmp, MAX_FILENAME, devtemplate, devname, lun);
	return fopen(dev, O_RDWR);
}
...
{
	return ioctl(h, VMEIO_SET_TIMEOUT, &timeout);
}

int encore_get_timeout(encore_handle h, int *timeout)
{
	return ioctl(h, VMEIO_GET_TIMEOUT, timeout);
}

int encore_wait(encore_handle h)
{
	struct vmeio_read_buf_s rb;
	return read(h, &rb, sizeof(rb));
}

int encore_read(encore_handle h, , unsigned int *value)
{
	unsigned int here;
	struct vmeio_riob_s riob;

	riob.
	ioctl(h, VMEIO_RAW_READ, value);
}
